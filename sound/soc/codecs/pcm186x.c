...
static int pcm186x_codec_probe(struct snd_soc_codec *codec)
{
	...
	if ((priv->type == PCM1864) || (priv->type == PCM1865)) {
		snd_soc_add_codec_controls(codec, pcm1865_snd_controls,
			ARRAY_SIZE(pcm1865_snd_controls));
		snd_soc_dapm_new_controls(dapm, pcm1865_dapm_widgets,
			ARRAY_SIZE(pcm1865_dapm_widgets));
	} else {
		snd_soc_add_codec_controls(codec, pcm1863_snd_controls,
			ARRAY_SIZE(pcm1863_snd_controls));
		snd_soc_dapm_new_controls(dapm, pcm1863_dapm_widgets,
			ARRAY_SIZE(pcm1863_dapm_widgets));
	}

	snd_soc_dapm_add_routes(dapm, pcm1865_dapm_routes, ARRAY_SIZE(pcm1865_dapm_routes));
			     ...
			     struct snd_soc_dai *dai)
{
	...
	case 32:
		...
		break;
	default:
    dev_err(codec->dev, "%s() error width %d\n", __func__, width);
		return -EINVAL;
	}
	...
	if (priv->is_tdm_mode) {
		...
		case 6:
			...
			break;
		default:
			dev_err(codec->dev, "%s() error channels %d\n", __func__, channels);
			return -EINVAL;
		}
...
static const struct snd_soc_codec_driver soc_codec_pcm186x = {
		  ...
		  struct regmap *regmap)
{
	...
	case PCM1865:
	case PCM1864:
		ret = snd_soc_register_codec(dev, &soc_codec_pcm186x,
					     &pcm1865_dai, 1);
		break;
	case PCM1863:
	case PCM1862:
	...
	dev_info(dev, "pcm186x_probe ok, type %d\n", type);
